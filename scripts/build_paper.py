...
p('方法学上需说明,Macrophage(20,280个)、CD8⁺T(23,486个)、内皮(7,520个)、B(25,660个)四类细胞各随机'
  '降采样至1,200个,与细胞数最少的周细胞(1,525个)对齐——这是CellPhoneDB官方1000次细胞标签置换检验方法'
  '的常规做法,细胞数越多置换检验耗时越长。更重要的是,降采样后的结果本身已经具有明确意义:上文全部7组'
  '目标配体-受体分子对在此样本量下均检出P<0.001的显著结果,说明降采样后的样本量已足以充分检出信号,'
  '进一步扩大样本量对结论的边际价值有限,故未再尝试计算成本高得多的全量细胞重新运行。GSE178341的细胞'
  '类型标注中周细胞为单一群体,未像外部图谱那样进一步区分BASP1⁺活化亚型,因此本节结果应理解为周细胞'
  '总体层面的信号复现。')
